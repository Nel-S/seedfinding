...
void initializeGlobals() {
	...
	for (uint_fast8_t i = 0; i < ringStartingIndex; ++i) {
		U_initClimateBoundsArray(NP_CONTINENTALNESS, U_getEffectiveContinentalness(FITNESS - U_SPAWN_FIRST_STAGE_VALS[i][U_spawn_table_fitness], false), FIFTIETH_PERCENTILE, Cdouble[i], sizeof(*Cdouble)/sizeof(**Cdouble));
	}
}
...
void *runWorker(void *workerIndex) {
	...
	U_manualBNinit(&bn, LARGE_BIOMES_FLAG);

	int ids[BIOME_CACHE_SIZE + 50000]; // HACK: BIOME_CACHE_SIZE needs to be properly fixed
	PerlinNoise *oct = bn.oct;
	double pxs[ringStartingIndex], pzs[ringStartingIndex];
	for (uint_fast8_t i = 0; i < ringStartingIndex; ++i) {
		pxs[i] = STATIC_CAST(int, U_SPAWN_FIRST_STAGE_VALS[i][U_spawn_table_x]);
		pzs[i] = STATIC_CAST(int, U_SPAWN_FIRST_STAGE_VALS[i][U_spawn_table_z]);
		if (DELAY_SHIFT) {
			pxs[i] = floor(pxs[i]/4.);
			pzs[i] = floor(pzs[i]/4.);
		}
	...
	do {
		...
		SpawnResult firstStageIndex, secondStageIndex;
		double fitness;
		if (!U_firstStageSpawnBounded(oct, FITNESS, false, LARGE_BIOMES_FLAG, &firstStageIndex, &fitness)) continue;
		...
		if (!U_secondStageSpawnBounded(oct, &firstStageIndex, fitness, FITNESS, false, LARGE_BIOMES_FLAG, &secondStageIndex, &fitness)) continue;
		Pos approxSpawn;
		if (areSpawnResultsIndices()) {
			approxSpawn.x = (STATIC_CAST(int, U_SPAWN_SECOND_STAGE_VALS[firstStageIndex.index][secondStageIndex.index][U_spawn_table_x]) & -16) + 8;
			approxSpawn.z = (STATIC_CAST(int, U_SPAWN_SECOND_STAGE_VALS[firstStageIndex.index][secondStageIndex.index][U_spawn_table_z]) & -16) + 8;
		} else {
			approxSpawn.x = (secondStageIndex.pos.x & -16) + 8;
			approxSpawn.z = (secondStageIndex.pos.z & -16) + 8;
		}
		...
		for (int dx = -RADIUS; dx <= RADIUS - 1; dx += 2*RADIUS - 1) {
			for (int dz = -RADIUS; dz <= RADIUS - 1; dz += 2*RADIUS - 1) {
				...
				// Continentalness
				npC = U_sampleClimate(NP_CONTINENTALNESS, oct, &px, &pz, LARGE_BIOMES_FLAG);
				if (npC > highestCont) {
					highestCont = npC;
