...
/*
	MUST DEFINE outputValue() in custom main() file. Also, localStartSeed and localSeedsToCheck can be overridden if desired.
*/
...
void *checkSeed(void *dat) {
	...
	for (uint64_t count = data->id; count < localSeedsToCheck; count += NUMBER_OF_THREADS) {
		...
		if (lowestCont > -1/AMPLITUDES[2]) continue;

		// TODO: Lazy, should remove at some point
		setBiomeSeed(&bn, seed, LARGE_BIOMES_FLAG);
		...
		for (int j = 0; j < 2*RADIUS; j++) {
			...
			for (int i = 0; i < 2*RADIUS; i++) {
				...
				voronoiAccess3D(getVoronoiSHA(seed), x, 63, z, &x4, &y4, &z4);
				x4 -= s.x;
				y4 -= 15;
				z4 -= s.z;
				int biome = ids[(int64_t)y4*s.sx*s.sz + (int64_t)z4*s.sx + x4];
