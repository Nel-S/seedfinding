...
void *checkSeed(void *dat) {
	...
	for (uint64_t count = data->id; count < SEEDS_TO_CHECK; count += NUMBER_OF_THREADS) {
		uint64_t seed = count + START_SEED;
		...
		for (uint_fast8_t i = 1; i < RING_STARTING_INDEX; ++i) {
			double pz = 0;
			if (!DELAY_SHIFT) {
		...
		uint32_t distSquared = approxSpawn.x * approxSpawn.x + approxSpawn.z * approxSpawn.z;
		...
		blocks away.*/
		if (distSquared >= MIN_RADIAL_DISTANCE * MIN_RADIAL_DISTANCE || abs(approxSpawn.x) >= MIN_AXIAL_DISTANCE || abs(approxSpawn.z) >= MIN_AXIAL_DISTANCE) printf("%" PRId64 "\t%d\t%d\t%.1f\n", seed, approxSpawn.x, approxSpawn.z, sqrt(distSquared));
		#else
		printf("%" PRId64 "\t%d\n", seed, minI);
		#endif
		skip: continue;
