...
def hashcode(string: str) -> int:
	""" The original hashcode function to reverse. """
	h = 0
	for character in string:
		h = 31 * h + ord(character)
	return h & 0xffffffff

def reverseHashcode(originalValue: int, maxOutputs: int | None = None) -> None:
	...
	Better tools already exist for this purpose (e.g. https://gist.github.com/pseudogravity/5a92ec51f5d97082358d06ef9b788e6b or https://github.com/hashcat/hashcat, which
	neither of which are restricted to exclusively-alphabetical strings), but this is at least my own personal take on the reversal problem."""
	...
	while not maxOutputs or outputCount < maxOutputs:
		...
		for offset in range(ceil(((thirtyOnePower - 1)//6*13 - originalValue)/(1 << 32)), int(((thirtyOnePower - 1)//15*61 - originalValue)//(1 << 32)) + 1):
			...
			for letterCase in range(minCombo, maxCombo + 1):
				...
				for i in caseBin + [0]*(stringLength - len(caseBin)):
					...
					premodHashCopy //= 31
				# Then if the case wasn't invalidated, reverse the output string (since we determined the coefficients above from the last term to the first), and print it. Then increment outputCount and break if appropriate.
				if invalidFlag: continue
				print(f'{backwardsOutput[::-1]} ( -> {hashcode(backwardsOutput[::-1])})', flush=True)
				outputCount += 1
				if maxOutputs and outputCount >= maxOutputs: break
			if maxOutputs and outputCount >= maxOutputs: break
